...
import (
	...
	clientset "github.com/ouyang-xlauncher/pipeline/pkg/client/clientset/versioned/typed/pipeline/v1alpha1"
...
func verifyExpectedOrder(ctx context.Context, t *testing.T, c clientset.TaskRunInterface) {
	...
	if len(taskRuns) != 5 {
		t.Fatalf("Expected 5 TaskRuns to have executed but only got start times for %d TaskRuns", len(taskRuns))
	}
